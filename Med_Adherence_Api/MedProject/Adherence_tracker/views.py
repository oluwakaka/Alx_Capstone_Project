...
class MyProfileView(APIView):
    ...
    def put(self, request):
        ...
        if user.role == "patient" and hasattr(user, "patient_profile"):
            ...
            ser = PatientProfileSerializer(user.patient_profile, data=request.data, partial=True)
            ser.is_valid(raise_exception=True)
            ser.save()
        if user.role == "doctor" and hasattr(user, "doctor_profile"):
            from .serializers import DoctorProfileSerializer
            ser = DoctorProfileSerializer(user.doctor_profile, data=request.data, partial=True)
            ser.is_valid(raise_exception=True)
            ser.save()
...
class AdherenceSummaryView(APIView):
    ...
    def get(self, request, patient_id):
        ...
        allowed = (
            u.role == "admin" or
            (u.role == "patient" and patient.user_id == u.id) or
            (u.role == "doctor" and u.doctor_profile.patients.filter(pk=patient.pk).exists())
        )
        if not allowed:
...
class NotificationSendView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        # minimal “send” — creates record (email/SMS integration could go here)
